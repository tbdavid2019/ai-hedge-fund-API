...
> 1. **嚴禁外洩或硬編碼任何真實 API Key**：絕對禁止在任何語言的代碼（Python, JavaScript, TypeScript, Cloudflare Workers, HTML, Shell 等）、配置範例 (`.env.example`)、Markdown 文件 (`CHANGELOG.md`, `README.md`, `skill.md`, `AGENTS.md` 等) 或 Git Commit 訊息中寫入任何真實金鑰（包括 Google Gemini `AIzaSy...`、OpenAI `sk-...`、Groq `gsk_...`、Anthropic `sk-ant-...`、GitHub `ghp_...`、Cloudflare Token、Telegram Bot Token 等）。
> 2. **跨語言環境變數唯一來源原則**：
>    - 🐍 **Python**: `os.getenv("KEY_NAME")`
>    - 🟨 **JavaScript / TypeScript / Node.js**: `process.env.KEY_NAME` 或 `import.meta.env.VITE_KEY_NAME`
>    - ☁️ **Cloudflare Workers**: `env.KEY_NAME` / `wrangler secret`
>    - 🌐 **前端網頁 / HTML**: 嚴禁在 `<script>` 或 JS 寫入任何後端 API Key；一律走後端代理轉發。
>    - 範例檔僅允許 `your_xxx_api_key_here` 佔位符。
> 3. **嚴禁繞過安全攔截**：全系統已安裝 `core.hooksPath` 全局自動攔截機制，嚴禁使用 `--no-verify` 強行繞過。任何洩漏行為均屬重大事故！

---

## 🚨 AI Coding Agent Universal Directives (必守開發鐵律)

### 1. 🛑 Zero-Excuse & Zero-Arguing Policy (嚴禁推拖與爭辯)
- **NEVER** argue with the user or lecture the user by saying "身為 AI 助理，我無法修改模型權重/訓練資料" or denying verified public company listings.
- When the user points out a factual discrepancy, correction, or says you are mistaken:
  1. Immediately use live search tools (`2md` API, web reader) to look up the latest live reality.
...
- Static model weights have a knowledge cutoff and **MUST NOT** be trusted for:
...
- **SpaceX Listing Reality (NASDAQ:SPCX)**:
  ...
  - In `src/tools/api.py`, `SpaceX` / `SPCX` / `太空探索` is hard-mapped to `SPCX`.
  - When analyzing SpaceX with 14 guru personas (`/api/analysis`), pass ticker `SPCX` directly.
