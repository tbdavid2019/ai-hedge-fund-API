...
- [x] 3.6 Configure production Compose to use a persistent mounted SQLite database path, migrate any existing in-container run store, and exclude database files from the image build context; verify prior runs survive service container replacement.
...
- [x] 4.3 Validate `selectedAnalysts` against the shared registry in synchronous, asynchronous, and grid API routes; return HTTP 400 for malformed or unknown keys and add route regression coverage.
- [x] 4.4 Extend deployment smoke checks to reject an unknown analyst key without creating a run; verify the response is HTTP 400.
